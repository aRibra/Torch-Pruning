...
from mmpretrain.models.utils.attention import WindowMSA
...
def load_model():
    weights = "/mnt/disks/ext/exps_swin_t/swin_t_tiny_in200/swin_t_tiny_in200/swin_t_tiny_in200/best_accuracy_top1_epoch_27.pth"
    cfg = "/mnt/disks/ext/repos/mmpretrain/configs/swin_transformer/swin-tiny_1xb256_in1k_224_1k.py"
    model = init_model(cfg, weights, device='cpu')
    model.train()
    # model.backbone.out_indices = (3, )
...
example_inputs = torch.randn([1, 3, 224, 224]).cpu()
...
pruner = tp.pruner.MetaPruner(
                ...
                pruning_ratio_dict = {model.backbone.norm3: 0, model.head.fc: 0, model.neck.gap: 0},
...
save_as = f"/mnt/disks/ext/swin_t_checkpoints/swin_t_backbone_Pruned_{int(pruning_ratio * 100)}.pth"
